...
internal abstract class CertificateManager
{
    ...
    public static MacOSCertificateManager Instance { get; } = new MacOSCertificateManager();
    ...
    protected internal abstract X509Certificate2 SaveCertificateCore(X509Certificate2 certificate, StoreName storeName, StoreLocation storeLocation);

    protected internal abstract void TrustCertificateCore(X509Certificate2 certificate);

    protected internal abstract bool IsExportable(X509Certificate2 c);

    protected internal abstract void RemoveCertificateFromTrustedRoots(X509Certificate2 certificate);

    protected internal abstract IList<X509Certificate2> GetCertificatesToRemove(StoreName storeName, StoreLocation storeLocation);
                ...
                switch (format)
                {
                    ...
                    case CertificateKeyExportFormat.Pem:
                        ...
                        else
                        {
                            ...
                            // To bypass it, we export the certificate to pem temporarily and then we import it and export it as unprotected internal PEM.
                            // TODO: cleanup cast: https://github.com/dotnet/aspnetcore/issues/41455
    ...
    protected internal virtual void RemoveCertificateFromUserStore(X509Certificate2 certificate)
    {
        try
        {
            if (Log.IsEnabled())
    ...
    protected internal virtual void RemoveCertificateFromUserStoreCore(X509Certificate2 certificate)
    {
